function varargout = CaronPro(varargin)
% CARONPRO MATLAB code for CaronPro.fig
%      CARONPRO, by itself, creates a new CARONPRO or raises the existing
%      singleton*.
%
%      H = CARONPRO returns the handle to a new CARONPRO or the handle to
%      the existing singleton*.
%
%      CARONPRO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CARONPRO.M with the given input arguments.
%
%      CARONPRO('Property','Value',...) creates a new CARONPRO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CaronPro_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CaronPro_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CaronPro

% Last Modified by GUIDE v2.5 03-Jul-2016 20:11:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CaronPro_OpeningFcn, ...
                   'gui_OutputFcn',  @CaronPro_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CaronPro is made visible.
function CaronPro_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CaronPro (see VARARGIN)

% Choose default command line output for CaronPro
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes CaronPro wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CaronPro_OutputFcn(hObject, eventdata, handles) 
